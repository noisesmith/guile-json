  ...
  (let getstring ((current (read-delimited "\\\"" fd 'split)))
    (case (cdr current)
      ((#\")
       (car current))
      ((#\\)
       (let ((newc (json:backslash fd))
	     (temp (read-delimited "\\\"" fd 'split)))
	 (list-set! temp 0 (string-append (car current) newc (car temp)))
	 (getstring temp)))
      (else
       (error "Error parsing JSON")))))

;; Helper function for getting the key-value pair from a
;; map / object / whatever you wanna call it
...
(define (json:read-bool fd)
  (case (read-char fd)
    ((#\t)
     (if (string=? "true"
		   (string #\t (read-char fd) (read-char fd) (read-char fd)))
	 #t))
    ((#\f)
     (if (string=? "false"
		   (string #\f
			   (read-char fd) (read-char fd)
			   (read-char fd) (read-char fd)))
	 #f))))

(define (json:read-null fd)
  (if (string=? "null" (string (read-char fd) (read-char fd) (read-char fd)
			       (read-char fd)))
      json:null
      (error "Error parsing JSON")))
